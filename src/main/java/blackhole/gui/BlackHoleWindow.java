...
public class BlackHoleWindow extends JFrame {
    ...
    public final void newGame() {
        ...
        logic.newGame();
        for (int i = 0; i < logic.getN(); i++) {            //gombok tulajdonságainak beállitasa
    ...
    private GameButton makeGameButton(int j, int i) {//játéktér mezõinek beállítása
    ...
    private final AbstractAction clickAction = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int x = gb.getPosX();
            int y = gb.getPosY();
            if (logic.playerTurn(y, x)) {                                           //csak az éppen soronkövetkezõ játékos  
                menuAction.setPosition(x, y);                                       // Å±rhajÃ³inál jÃ¶n elÅ‘ a popupmenu
                for (int i = 0; i < 4; i++) {
                    components[i].setEnabled(true);
                }
                for (Integer integer : logic.passableWays(y, x)) {                  //azokba az irányokba, amellyekbe nem mehet az Å±rhajÃ³ ott a
                    components[integer].setEnabled(false);                          //popupmenu megfelelõ eleme(i) "kikapcsolÃ¡sra" kerÃ¼l(nek)
                }
                popupMenu.show(gb, x, y);
            }
        }
    };
    AbstractAction newGameaction = new AbstractAction() {//Új játék létrehozása, a pálya megfelelõ méretének a beállítása
